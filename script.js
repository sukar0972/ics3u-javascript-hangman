...
function generateRandomWord() {
    //Generate a random number from 0 to the length of the array; then go to that index
    var word = words[Math.floor(Math.random() * words.length)];
    return word;
}

/**
 * Draws text on the canvas using the parameters given
 * @param {String} output //What the user wants to output
 * @param {int} x //The x position of the text
 * @param {int} y //The y position of the text
 * @param {String} color //The color of the text
 * @param {int} size //The size in px of the text
 * @param {String} font //The font of the text
 */
function drawText(output,x,y,color,size,font) {
    var sizeFont = size + "px "+ font; //Example "62px arial"
    ctx.fillStyle = color;
    ctx.font = sizeFont;
...
function easyMode(word) {
    ...
    for (i = 0; i < gameWord.length; i++) {
        ctx.lineWidth = 5;
        ctx.beginPath();
        ctx.moveTo(675 + i * 75, 220);
        ctx.lineTo(700 + i * 75, 220);
        ctx.stroke();
    }

    //Initiate Variables
    var userInput = ""; //The letter the user inputs
    var lives = 10;
    var attempts = 0;
    var lettersGuessed = 0;
    let guessResults = [];
    
    //Show the lives left in the top corner
    showLivesScore(lives);

    //On keypress
    document.onkeydown = function (e) {

        //Make a white rectangle below the letter they are guessing so they do not overlap
        ctx.fillStyle = "white";
        ctx.fillRect(684, 400, 70, 70)
        ctx.fillStyle = "blue";


        //If the selected key is a valid letter (not a modifier, etc)
        if (alphabet.includes(e.key.toLowerCase())  && guessResults[1]!=gameWord.length && lives>0) {
            ...
            drawText(userInput,700,450,"brown",68,"arial")
        }
...
/**
 * Draw the appropriate part of the hangman depending on how many lives they have
 * Handles game over 
 * @param {int} lives 
 * @returns 
 */

function drawHangman(lives,word) {
    if(lives == 9) {
        drawGallow();
    } else if(lives == 8) {
        drawHead();
...
/**
 * Checks and sees if the letter the user inputs is located in the word and will output it to the correct location depending on it
 * @param {String} userInput //The letter the user want to enter
 * @param {String array} gameWord //The entire word of the letter in an array
 * @returns correctGuess[0,1] 0: if the letter the user entered is true; 1 - how many letters the user guessed
 */
function checkLetters(userInput, gameWord) {

    //Array with all the positions of letters in the word
    let letterPos = [];

    //Default value for if the guess was correct or not and the amount of words that they guessed
    ...
    if (gameWord.includes(userInput)) {
        for (var i = 0; i < gameWord.length; i++) {
            //If the letter is contained in the word add the position to an array
            if (gameWord[i] === userInput) {
                ...
                //Draw the letter on the canvas above the dashed lines. letterPos indicates where in the canvas it should go
                drawText(gameWord[letterPos],675+letterPos*75,200,"blue",68,"arial");


                //If there are multiple occurences of the same letter. "apple" => 1 => 1,2
                if (letterPos.length > 1) {
                    //Set the second occurence to second indice of the array letterPos
                    var secondOc = letterPos[1];
                    //Draw the second letter on the canvas with the position of the second occ
                    drawText(gameWord[secondOc],675+secondOc*75,200,"blue",68,"arial");
                }

                //Set the guessed letters to something that cannot be guessed
                gameWord[i] = undefined;
            }
        }
        //If the letter was not contained in the word
    } else {
        ...
        //Make it so there can only be one of each in the array. ["a","b","a"] = > ["a","b"]
        ...
        //Display the wrong letters in the word bank
        drawText(exclusiveWrongLetters,600,600,"red",68,"arial");
    }
    return correctGuess; //Returns if the letter they gave was in the word, and the amount of letters they had guessed
}

/**
 * Using the free dicitonary api, get the defintion of the word as a hint
 * @param {String} word 
 */
function getHint(word) {
    ...
    .then(data => {
      //Get the def of the word from the url
      let definition = data[0].meanings[0].definitions[0].definition;
      drawText("Hint: "+definition,10,70,"black",12,"arial");
    })
...
/**
 * Shows the scoreboard of the user
 * @param {int} lives //How many lives they have left
 * @param {int} score  //The users score
 */
function showLivesScore(lives) {
    //Show Lives
    ctx.fillStyle = "white";
    ctx.fillRect(950, 00, 70, 50);

    //Draw text for lives they have
    drawText("Lives:",700,50,"red",48,"helvetica");
    drawText(lives,950,50,"red",48,"helvetica");
}

/**
 * Shows the winning screen
 * @param {int} attempts  //Used to show the number of attempts it took them
 */
function winner(attempts) {
    ...
    //Change the msg based on the amount of attempts it took them
    if(attempts == 1) {
        msg = "Unreal!"
    } else if(attempts <=3) {
        msg = "Fantastic!"
    ...
    drawText(msg,420,300,"white",90,"arial");
    drawText("You did it in "+attempts+" attempts.",290,510,"white",62,"arial");
    drawText("Wanna play again?",50,720,"white",24,"arial");
    drawText("Hit Enter!",50,770,"white",24,"arial");
...
/**
 * Game over screen. Show the correct word 
 * @param {String} word 
 */
function gameOver(word) {
    ...
    drawText("Game Over!",300,100,"red",72,"arial");
    drawText("The word was: "+ word,300,300,"red",68,"arial");
    ...
    //If they hit enter
    document.onkeydown = function (e) {
        if(e.key == "Enter") { //Reload the page
            window.location.reload();
        }
    }
...
/**
 * Draws the gallow of the hangman game
 */
function drawGallow() {
    ctx.lineWidth = 8;
    ctx.beginPath();
...
/**
 * Draw the face of the stick figure
 * @param {int} mood //Facial expression changes based on this value
 */
function drawFace(mood) {
    ...
    ctx.beginPath();
    ctx.arc(465, 290, 5, 0, 2 * Math.PI, true); // right eye
    ctx.fillStyle = "black";
    ctx.fill();
...
/**
 * Draw one of the arms of the stick figure
 */
function drawArmOne() {
    ctx.lineWidth = 5;
    ctx.beginPath();
...
/**
 * Draw another one of the arms on the stick figure
 */
function drawArmTwo() {
    ctx.lineWidth = 5;
    ctx.beginPath();
...
/**
 * Draw one of the legs of the stick figure
 */
function drawLegOne() {
    ctx.lineWidth = 5;
    ctx.beginPath();
...
/**
 * Draw the other leg of the stick figure
 */
function drawLegTwo() {
    ctx.lineWidth = 5;
    ctx.beginPath();
...
/**
 * Show the mouse pos. Useful to know where to place elements
 */
